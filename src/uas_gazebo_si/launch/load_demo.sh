...
usage() {
  ...
  echo "  -w  Specify the world name (default: serf)"
...
# Package directory
PACKAGE_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd) # Go up one level
WORLD_PATH="$PACKAGE_DIR/worlds/${WORLD_NAME}/${WORLD_NAME}.sdf"

# Ensure the PX4 directory exists
if [ ! -d "$PX4_DIR" ]; then
  echo "Error: PX4 directory does not exist."
  exit 1
fi
...
PX4_GZ_STANDALONE=1 PX4_SYS_AUTOSTART=4001 PX4_SIM_MODEL=gz_x500 PX4_GZ_WORLD="$WORLD_NAME" PX4_GZ_MODEL_POSE="$PX4_GZ_MODEL_POSE" $PX4_DIR/build/px4_sitl_default/bin/px4 &
...
source ~/ros2_ws/install/setup.bash  # Modify this path based on your workspace location
