...
class YoloNode(Node):
    def __init__(self):
        ...
        pkg_share = get_package_share_directory('yolov5_ros2')
        custom_weights_path = os.path.join(pkg_share, 'models', 'best.pt')  # Make sure the path is correct
        if os.path.exists(custom_weights_path):
            # self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=custom_weights_path, source='local', force_reload=True)
